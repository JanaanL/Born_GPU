...
bool tmute::adjoint(bool add,std::shared_ptr<oc_float> model, std::shared_ptr<oc_float>data){
	...
	std::shared_ptr<hypercube_float >d( hypercube_float(as));
	std::shared_ptr<hypercube_float>m(hypercube_float(as));
	dat->_file->seekTo(0,0);
	mod->_file->seekTo(0,0);
	long long n123=a1.n*a2.n*a3.n;
	for(int i5=0; i5 < a5.n; i5++) {
		for(int i4=0; i4 < a4.n; i4++) {
			dat->file->readFloatStream(d->vals,n123);
			mod->file->readFloatStream(d->vals,n123);
...
bool tmute::forward(bool add, std::shared_ptr<oc_float>model, std::shared_ptr<oc_float> data){
	std::shared_ptr<oc_float> dat = std::dynamic_pointer_cast<oc_float>( data);
	std::shared_ptr<oc_float> mod = std::dynamic_pointer_cast<oc_float>( model);
	axis a1=dat->getAxis(1);
	axis a2=dat->getAxis(2);
	axis a3=dat->getAxis(3);
	axis a4=dat->getAxis(4);
	axis a5=dat->getAxis(5);
